...
**Accessed from:** CommandInputs.addTabCommandInput

## Methods

### activate() -> boolean
Sets this to be the currently activated (selected) tab.
- **Returns** (boolean): Returns true if the TabCommandInput was successfully activated.
...
### children : CommandInputs [read-only]
...
### resourceFolder : string [read-only]
Gets the folder that contains the icon for the tab. If no name is specified (no text on tab), a resourceFolder containing the icon needs to be provided. Icons can be defined using either PNG or SVG files. More information about icons can be found in the user manual topic User Interface Customization.
