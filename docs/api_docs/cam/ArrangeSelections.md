...
**Accessed from:** CAMArrangeParameterValue.getArrangeSelections
...
- **Returns** (ArrangeSelection): Returns newly created ArrangeSelection.

### item(index: uinteger) -> ArrangeSelection
Function that returns the specified arrange selection object using an index into the collection.
- **index** (uinteger): The index of the item within the collection to return. The first item in the collection has an index of 0.
- **Returns** (ArrangeSelection): Returns the specified item or null if an invalid index was specified.
...
### removeByObject(selection: ArrangeSelection)
