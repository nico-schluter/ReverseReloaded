...
**Accessed from:** CAM.allMachines, Machine.create, MachineLibrary.childMachines, MachineLibrary.machineAtURL, MachineQuery.execute, NCProgram.machine, PrintSettingQuery.machine, Setup.machine, SetupInput.machine
...
### create(machineInput: MachineInput) -> Machine
...
- **Returns** (Machine): Returns the newly created machine in a valid state.
...
- **Returns** (boolean): True if the machines are equivalent

## Properties

### capabilities : MachineCapabilities [read-only]
Gets the capabilities of the machine.

### description : string [read-write]
Gets and sets the description of the machine.

### elements : MachineElements [read-only]
Gets the list of elements that make up this machine.

### hasPost : boolean [read-only]
Checks if the machine has a post.

### hasSimulationModel : boolean [read-only]
