...
**Accessed from:** AdditiveSetupUtility.assignPartsToBodyPreset, DocumentToolLibrary.operationsByTool, Operations.item, Operations.itemByName, Operations.itemByOperationId
...
### generatingProgress : string [read-only]
...
### isGenerating : boolean [read-only]
...
### isToolpathValid : boolean [read-only]
...
### operationState : OperationStates [read-only]
...
### strategyType : OperationStrategyTypes [read-only]
