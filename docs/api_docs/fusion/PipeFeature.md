...
**Accessed from:** PipeFeature.createForAssemblyContext, PipeFeature.nativeObject, PipeFeatures.add, PipeFeatures.item, PipeFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> PipeFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (PipeFeature): Returns the proxy object or null if this isn't the NativeObject.
...
### nativeObject : PipeFeature [read-only]
...
### sectionSize : ModelParameter [read-only]
Gets and sets the section size of the Pipe.

### sectionThickness : ModelParameter [read-only]
