...
**Accessed from:** SketchEllipse.createForAssemblyContext, SketchEllipse.nativeObject, SketchEllipses.add, SketchEllipses.item
...
### createForAssemblyContext(occurrence: Occurrence) -> SketchEllipse
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (SketchEllipse): Returns the proxy object or null if this isn't the NativeObject.
...
### majorAxisLine : SketchLine [read-only]
...
### minorAxisLine : SketchLine [read-only]
...
### nativeObject : SketchEllipse [read-only]
...
### worldGeometry : Ellipse3D [read-only]
