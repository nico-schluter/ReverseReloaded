...
**Accessed from:** UntrimFeature.createForAssemblyContext, UntrimFeature.nativeObject, UntrimFeatures.add, UntrimFeatures.item, UntrimFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> UntrimFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (UntrimFeature): Returns the proxy object or null if this is not the NativeObject.
...
### extensionDistance : ModelParameter [read-only]
...
### nativeObject : UntrimFeature [read-only]
