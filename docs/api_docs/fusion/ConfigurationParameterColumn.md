...
**Accessed from:** ConfigurationColumns.addParameterColumn, ConfigurationParameterCell.parentColumn
...
### getCell(rowIndex: uinteger) -> ConfigurationParameterCell
...
- **Returns** (ConfigurationParameterCell): Returns the specified cell if successful and null if an invalid index was specified.

### getCellByRowId(rowId: string) -> ConfigurationParameterCell
Gets the cell in this column at the row specified by its ID.
- **rowId** (string): The ID of the row to return the cell for.
- **Returns** (ConfigurationParameterCell): Returns the specified cell if successful and null if the id is not found.

### getCellByRowName(rowName: string) -> ConfigurationParameterCell
Gets the cell in this column at the row specified by its name.
- **rowName** (string): The name of the row to return the cell for.
- **Returns** (ConfigurationParameterCell): Returns the specified cell if successful and null if the name is not found.
