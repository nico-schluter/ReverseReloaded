...
**Accessed from:** SplitFaceFeature.createForAssemblyContext, SplitFaceFeature.nativeObject, SplitFaceFeatures.add, SplitFaceFeatures.item, SplitFaceFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> SplitFaceFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (SplitFaceFeature): Returns the proxy object or null if this is not the NativeObject.
...
### setAsAlongVectorSplitType(splittingTool: Base, directionEntity: Base) -> boolean
...
### setAsClosestPointSplitType(splittingTool: Base) -> boolean
Sets the split type to be a curve that defined by projecting the splitting curve to the closest point on the surface.

To use this method, you need to position the timeline marker to immediately before this feature. This can be accomplished using the following code: thisFeature.timelineObject.rollTo(True)
- **splittingTool** (Base): Input entity(s) that defines the splitting tool. The splitting tool can be a single entity or an ObjectCollection containing faces or sketch curves. If faces are input, the edges of the face are used as the splitting tool.
- **Returns** (boolean): Returns true if setting the closest point split type was successful.

### setAsSurfaceIntersectionSplitType(splittingTool: Base, isSplittingToolExtended: boolean) -> boolean
...
### directionEntity : Base [read-only]
...
### nativeObject : SplitFaceFeature [read-only]
...
### splittingTool : ObjectCollection [read-only]
