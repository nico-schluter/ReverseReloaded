...
**Accessed from:** ConfigurationColumns.addVisibilityColumn, ConfigurationVisibilityCell.parentColumn
...
### getCell(rowIndex: uinteger) -> ConfigurationVisibilityCell
...
- **Returns** (ConfigurationVisibilityCell): Returns the specified cell if successful and null if an invalid index was specified.

### getCellByRowId(rowId: string) -> ConfigurationVisibilityCell
Gets the cell in this column at the row specified by its ID.
- **rowId** (string): The ID of the row to return the cell for.
- **Returns** (ConfigurationVisibilityCell): Returns the specified cell if successful and null if the id is not found.

### getCellByRowName(rowName: string) -> ConfigurationVisibilityCell
Gets the cell in this column at the row specified by its name.
- **rowName** (string): The name of the row to return the cell for.
- **Returns** (ConfigurationVisibilityCell): Returns the specified cell if successful and null if the name is not found.
