...
**Accessed from:** SketchFixedSpline.createForAssemblyContext, SketchFixedSpline.nativeObject, SketchFixedSplines.addByNurbsCurve, SketchFixedSplines.item
...
### createForAssemblyContext(occurrence: Occurrence) -> SketchFixedSpline
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (SketchFixedSpline): Returns the proxy object or null if this isn't the NativeObject.
...
### replaceGeometry(nurbsCurve: NurbsCurve3D) -> boolean
...
### nativeObject : SketchFixedSpline [read-only]
