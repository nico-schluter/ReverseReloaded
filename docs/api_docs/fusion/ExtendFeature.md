...
**Accessed from:** ExtendFeature.createForAssemblyContext, ExtendFeature.nativeObject, ExtendFeatures.add, ExtendFeatures.item, ExtendFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> ExtendFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (ExtendFeature): Returns the proxy object or null if this is not the NativeObject.
...
### setInputEntities(edges: ObjectCollection, isChainingEnabled: boolean) -> boolean
...
### edges : ObjectCollection [read-only]
...
### extendAlignment : SurfaceExtendAlignment [read-write]
Gets and sets surface extend alignment to use.

To set this property, you need to position the timeline marker to immediately before this feature. This can be accomplished using the following code: thisFeature.timelineObject.rollTo(True)

### extendType : SurfaceExtendTypes [read-write]
...
### isChainingEnabled : boolean [read-only]
...
### nativeObject : ExtendFeature [read-only]
