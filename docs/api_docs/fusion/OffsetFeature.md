...
**Accessed from:** OffsetFeature.createForAssemblyContext, OffsetFeature.nativeObject, OffsetFeatures.add, OffsetFeatures.item, OffsetFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> OffsetFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (OffsetFeature): Returns the proxy object or null if this is not the NativeObject.
...
### setInputEntities(entities: ObjectCollection, isChainSelection: boolean) -> boolean
...
- **isChainSelection** (boolean): A boolean value for setting whether or not faces that are tangentially connected to the input faces (if any) will also be offset. The default value is true.
...
### isChainSelection : boolean [read-only]
...
### nativeObject : OffsetFeature [read-only]
...
### operation : FeatureOperations [read-only]
