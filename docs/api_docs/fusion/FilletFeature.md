...
**Accessed from:** FilletFeature.createForAssemblyContext, FilletFeature.nativeObject, FilletFeatures.add, FilletFeatures.addFullRoundFillet, FilletFeatures.addRuleFillet, FilletFeatures.item, FilletFeatures.itemByName
...
- **Returns** (boolean): Returns true if the conversion was successful.

### createForAssemblyContext(occurrence: Occurrence) -> FilletFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (FilletFeature): Returns the proxy object or null if this isn't the NativeObject.
...
### edgeSets : FilletEdgeSets [read-only]
...
### filletFeatureType : FilletFeatureTypes [read-only]
Returns the FilletFeatureTypes indicating this fillet feature type.

### fullRoundFilletFaceSets : FullRoundFilletFaceSets [read-only]
...
### nativeObject : FilletFeature [read-only]
...
### ruleFilletSettings : RuleFilletSettings [read-only]
