...
**Accessed from:** GeometricConstraints.addMidPoint, MidPointConstraint.createForAssemblyContext, MidPointConstraint.nativeObject
...
### createForAssemblyContext(occurrence: Occurrence) -> MidPointConstraint
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (MidPointConstraint): Returns the proxy object or null if this isn't the NativeObject.
...
### midPointCurve : SketchCurve [read-only]
Returns the curve defining the midpoint.

### nativeObject : MidPointConstraint [read-only]
