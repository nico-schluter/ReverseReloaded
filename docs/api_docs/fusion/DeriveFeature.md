...
**Accessed from:** BRepBody.deriveFeature, Canvas.deriveFeature, ConstructionAxis.deriveFeature, ConstructionPlane.deriveFeature, ConstructionPoint.deriveFeature, Decal.deriveFeature, DerivedParameter.deriveFeature, DeriveFeature.createForAssemblyContext, DeriveFeature.nativeObject, DeriveFeatures.add, DeriveFeatures.item, DeriveFeatures.itemByName, Occurrence.deriveFeature, Sketch.deriveFeature
...
- **Returns** (boolean): Returns true if the break link was successful.

### [static] classType() -> string
Static function that all classes support that returns the type of the class as a string. The returned string matches the string returned by the objectType property. For example if you have a reference to an object and you want to check if it's a SketchLine you can use myObject.objectType == fusion.SketchLine.classType().
- **Returns** (string): Returns a string indicating the type of the object.

### createForAssemblyContext(occurrence: Occurrence) -> DeriveFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (DeriveFeature): Returns the proxy object or null if this isn't the NativeObject.
...
### getDerivedEntity(sourceEntity: Base) -> Base
...
- **Returns** (Base): Returns the derived entity for the source entity or null if it failed.

### getSourceEntity(derivedEntity: Base) -> Base
...
- **Returns** (Base): Returns the source entity for the derived entity or null if it failed.

### setSourceEntities(entities: Base[], excludedEntities: Base[])
...
### documentReference : DocumentReference [read-only]
Returns the DocumentReference for this derived design. Through the DocumentReference object you can modify the version and get other information associated with the reference.
...
### excludedEntities : array [read-only]
...
Gets or sets whether all feature parameters from all derived components from the source design are derived or not.
...
### nativeObject : DeriveFeature [read-only]
...
### sourceEntities : array [read-only]
