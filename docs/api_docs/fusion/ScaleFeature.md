...
**Accessed from:** ScaleFeature.createForAssemblyContext, ScaleFeature.nativeObject, ScaleFeatures.add, ScaleFeatures.item, ScaleFeatures.itemByName
...
### createForAssemblyContext(occurrence: Occurrence) -> ScaleFeature
Creates or returns a proxy for the native object - i.e. a new object that represents this object but adds the assembly context defined by the input occurrence.
- **occurrence** (Occurrence): The occurrence that defines the context to create the proxy in.
- **Returns** (ScaleFeature): Returns the proxy object or null if this isn't the NativeObject.
...
### setToUniform(scaleFactor: ValueInput) -> boolean
Calling this method will change to a uniform scale. The isUniform is set to true if successful.

To use this method, you need to position the timeline marker to immediately before this feature. This can be accomplished using the following code: thisFeature.timelineObject.rollTo(True)
- **scaleFactor** (ValueInput): A ValueInput object that defines the scale factor.
- **Returns** (boolean): Returns true if successful.
...
### nativeObject : ScaleFeature [read-only]
...
### scaleFactor : ModelParameter [read-only]
...
### xScale : ModelParameter [read-only]
...
### yScale : ModelParameter [read-only]
...
### zScale : ModelParameter [read-only]
